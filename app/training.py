...
async def _queue_object(db: Database, runner: QueueRunner) -> dict:
    queued_row = await db.fetch_one("SELECT COUNT(*) AS n FROM training_jobs WHERE status = 'queued'")
    running_row = await db.fetch_one("SELECT id FROM training_jobs WHERE status = 'running' LIMIT 1")
    return {
        "state": runner.state,
...
async def _insert_job(
    ...
    *,
    ...
    stages = [
        {"key": key, "status": "skipped" if skip_cache_stages and key != "train" else "pending"}
    ...
    async with db.write_lock:
        ...
        await db.connection.execute(
            ...
            "dataset_config_toml, values_json, status, queue_position, stages_json, created_at) "
            "VALUES (?, ?, ?, ?, ?, ?, 'queued', ?, ?, ?)",
            (
                job_id,
                name,
...
async def create_job(payload: TrainingJobRequest, request: Request) -> dict:
    ...
    if shutil.which("accelerate") is None:
        raise HTTPException(status_code=503, detail="accelerate is not available on the server")
    if shutil.which("python") is None:
        raise HTTPException(status_code=503, detail="python is not available on the server")

    musubi_path = resolved_paths["musubiPath"]
    if not musubi_path.is_dir():
        raise HTTPException(status_code=503, detail="musubi-tuner directory is not available")
    required_scripts = [
        profile.cache_commands[0].script,
        profile.cache_commands[1].script,
        profile.trainer,
    ]
    missing_scripts = [name for name in required_scripts if not (musubi_path / name).is_file()]
    if missing_scripts:
        raise HTTPException(
            status_code=503,
...
@router.get("/jobs")
async def list_jobs(request: Request, status: str | None = Query(default=None)) -> list[dict]:
...
async def cancel_job(job_id: str, request: Request) -> dict:
    ...
    async with runner.claim_lock:
        ...
        if status in {"completed", "failed"}:
            raise HTTPException(status_code=409, detail=f"A {status} job can no longer be cancelled")
