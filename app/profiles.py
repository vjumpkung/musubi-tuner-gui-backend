...
TRAINING_PROFILES: dict[str, TrainingProfile] = {
    ...
    'framepack': TrainingProfile(
        ...
        model_fields=(
            ...
            ProfileField(key='textEncoder1', train_flag='--text_encoder1', required=True),
            ProfileField(key='textEncoder2', train_flag='--text_encoder2', required=True),
            ProfileField(key='imageEncoder', train_flag='--image_encoder', required=True)
        ),
    ...
    'wan-22': TrainingProfile(
        ...
        cpu_threads=16,
        task_options=('t2v-A14B', 'i2v-A14B', 't2v-1.3B', 't2v-14B', 'i2v-14B', 't2i-14B')
    ),
    ...
    'flux-2': TrainingProfile(
        ...
        selectors=(
            Selector(
                ...
                options=('dev', 'klein-4b', 'klein-base-4b', 'klein-9b', 'klein-base-9b')
            ),
        ),
        memory_flags=(
            MemoryFlag(key='fp8Base', flag='--fp8_base'),
    ...
    'hidream-o1': TrainingProfile(
        ...
        selectors=(
            Selector(key='modelType', train_flag='--model_type', options=('full', 'dev')),
        ),
        memory_flags=(
            MemoryFlag(key='pinnedBlockSwap', flag='--use_pinned_memory_for_block_swap'),
        ),
    ...
    'kandinsky-5': TrainingProfile(
        ...
        model_fields=(
            ProfileField(key='dit', train_flag='--dit', required=True),
            ProfileField(key='vae', train_flag='--vae', required=True),
            ProfileField(key='textEncoderQwen', train_flag='--text_encoder_qwen', required=True),
            ProfileField(key='textEncoderClip', train_flag='--text_encoder_clip', required=True)
        ),
